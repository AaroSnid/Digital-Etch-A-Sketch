...
int lis3dh_configure_shake_to_erase(lis3dhtr_cfg_t *hw_cfg, bool enable) {
    ...
    if (enable) {
        // ROUTE HIGH-PASS FILTER TO INT1 ENGINE
        uint8_t ctrl_reg2 = 0x01; 
        if (spi_write_data(hw_cfg, CTRL_REG2, &ctrl_reg2, 1) != 0) return -1;

        // CONFIGURE INT1 FOR LATERAL SHAKING (DYNAMIC INTERRUPT)
        uint8_t int1_cfg = 0x0A; // X-High and Y-High
        if (spi_write_data(hw_cfg, INT1_CFG, &int1_cfg, 1) != 0) return -1;

        uint8_t int1_ths = 0x2A; // Balanced shake threshold (~0.6g)
        if (spi_write_data(hw_cfg, INT1_THS, &int1_ths, 1) != 0) return -1;

        uint8_t int1_dur = 0x00; // Trigger instantly on a shake peak
        if (spi_write_data(hw_cfg, INT1_DURATION, &int1_dur, 1) != 0) return -1;

        // CONFIGURE INT2 FOR Z-AXIS POSITION (STATIC LEVEL PIN)
        uint8_t int2_cfg = 0xD0; // AOI = 1, 6D = 1, ZLIE = 1 (6D Position Mode)
        if (spi_write_data(hw_cfg, INT2_CFG, &int2_cfg, 1) != 0) return -1;

        uint8_t int2_ths = 0x20; // Stable orientation threshold (~0.5g)
        if (spi_write_data(hw_cfg, INT2_THS, &int2_ths, 1) != 0) return -1;

        uint8_t int2_dur = 0x00; // Trigger instantly on orientation change
        if (spi_write_data(hw_cfg, INT2_DURATION, &int2_dur, 1) != 0) return -1;

        // LATCH AND PIN ROUTING MECHANICS
        uint8_t ctrl_reg5 = 0x08; // Latch INT1 shake events
        if (spi_write_data(hw_cfg, CTRL_REG5, &ctrl_reg5, 1) != 0) return -1;

        uint8_t ctrl_reg3 = 0x40; // Map INT1 to physical pin
        if (spi_write_data(hw_cfg, CTRL_REG3, &ctrl_reg3, 1) != 0) return -1;

        uint8_t ctrl_reg6 = 0x20; // Map INT2 to physical pin
